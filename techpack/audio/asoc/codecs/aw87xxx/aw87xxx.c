...
#ifdef OPLUS_ARCH_EXTENDS
LIST_HEAD(g_aw87xxx_device_list);
#else
...
unsigned char g_pa_voltage_addr[AW87XXX_CHIP_TYPE_MAX] = {
	[AW87XXX_339] = AW87339_REG_CPOVP,
	[AW87XXX_359] = AW87359_REG_CPOVP,
	[AW87XXX_369] = AW87369_REG_CPOVP,
};
#endif
...
**********************************************************/
#ifdef OPLUS_ARCH_EXTENDS
int aw87xxx_i2c_write(struct aw87xxx *aw87xxx,
...
void aw87xxx_hw_on(struct aw87xxx *aw87xxx)
...
#ifdef OPLUS_FEATURE_SPEAKER_MUTE
void aw87xxx_hw_off(struct aw87xxx *aw87xxx)
#else /* OPLUS_FEATURE_SPEAKER_MUTE */
...
#ifdef OPLUS_AUDIO_PA_BOOST_VOLTAGE
	aw87xxx_scene_update_set_spk_voltage(aw87xxx);
				     ...
				     struct device_node *np)
{
	...
	aw_dev_info(aw87xxx->dev, "%s enter, dev_i2c%d@0x%02X\n", __func__,
		    aw87xxx->i2c_seq, aw87xxx->i2c_addr);
	snprintf(aw87xxx->bus_num, sizeof(aw87xxx->bus_num) + 1, "%u",
		 aw87xxx->i2c_seq);
...
static int aw87xxx_update_cfg_name(struct aw87xxx *aw87xxx)
{
	...
	memset(aw87xxx->cfg_name, '\0', sizeof(aw87xxx->cfg_name));
	...
	/*vamx bin */
	memset(aw87xxx->vmax_cfg_name, '\0', sizeof(aw87xxx->vmax_cfg_name));
	if (aw87xxx->channel[0] != '-') {
...
//#endif OPLUS_ARCH_EXTENDS
    ...
    */
	...
	if (speaker_device != NULL) {
		...
		speaker_device->chipset = aw87xxx->product;
		speaker_device->type = (1 << aw87xxx->pa_channel);
		if (aw87xxx->pa_channel == OPLUS_PA_CHANNEL_LEFT) {
			speaker_device->speaker_voltage = AW87XXX_SPK_VOLTAGE_DEFAULT;
		} else if (aw87xxx->pa_channel == OPLUS_PA_CHANNEL_RIGHT) {
			speaker_device->speaker_voltage = AW87XXX_RCV_VOLTAGE_DEFAULT;
		}
		speaker_device->speaker_enable_set = aw87xxx_enable;
		speaker_device->speaker_enable_get = aw87xxx_get_status;
		pr_info("%s, %d, aw87xxx->product = %d\n", __func__, __LINE__, aw87xxx->product);
		if (aw87xxx->product == AW87XXX_339 || aw87xxx->product == AW87XXX_359 || aw87xxx->product == AW87XXX_369) {
		...
		#ifdef OPLUS_FEATURE_SPEAKER_MUTE
		speaker_device->speaker_mute_set = aw87xxx_force_mute_set;
		#else /* OPLUS_FEATURE_SPEAKER_MUTE */
		...
		pr_info("%s, %d, oplus_register end\n", __func__, __LINE__);
...
static int aw87xxx_i2c_remove(struct i2c_client *client)
{
	...
	if (aw87xxx == NULL) {
		pr_err("%s, aw87xxx == NULL", __func__);
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
	if (aw87xxx != NULL) {
		struct oplus_spk_dev_node* spk_dev_node = aw87xxx->oplus_dev_node;
...
static void aw87xxx_i2c_shutdown(struct i2c_client *client)
{
	...
	if (aw87xxx->product != AW87XXX_339
		...
		&& aw87xxx->product != AW87XXX_519
		&& aw87xxx->product != AW87XXX_559) {
		pr_info("%s aw87xxx->product is not matchecd!\n", __func__);
		return;
	}
