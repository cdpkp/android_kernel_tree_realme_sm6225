...
extern int oplus_dc_dimlayer_bl_alpha_v2;
...
extern int dsi_panel_parse_oplus_apollo_config(struct dsi_panel *panel);
...
static int oplus_get_panel_brightness_to_alpha(void)
{
	...
	if (apollo_backlight_enable) {
		...
		if (index >= 0) {
			DSI_DEBUG("[%s] index = %d, panel_level = %d", __func__, index,
...
int dsi_panel_parse_oplus_bl_config(struct dsi_panel *panel)
{
	int rc = 0;
	u32 val = 0;
	struct dsi_parser_utils *utils = &panel->utils;
	...
	if (rc) {
		DSI_DEBUG("[%s] dsi-bl-hbm-min-level undefine, set to zero \n",
			 panel->name);
		panel->bl_config.bl_hbm_min_level = 0;
	} else {
	...
	if (rc) {
		DSI_DEBUG("[%s] dc backlight unspecified, defaulting to default level 260\n",
			 panel->name);
		oplus_dc_dimlayer_bl_alpha_v2 = 260;
		panel->dc_last_level = 260;
	} else {
		oplus_dc_dimlayer_bl_alpha_v2 = val;
		panel->dc_last_level = val;
	}

	panel->is_dc_support = utils->read_bool(utils->data, "oplus,dc-dimming-support");
...
int dsi_panel_parse_oplus_feature_config(struct dsi_panel *panel)
{
	int ret = 0;
	struct dsi_parser_utils *utils = &panel->utils;

	/* add for spr config */
	panel->oplus_priv.is_spr_support = utils->read_bool(utils->data, "oplus,spr-support");
	pr_info("[%s]spr mode support: %s", __func__, panel->oplus_priv.is_spr_support ? "Yes" : "Not");
	...
	ret = utils->read_u32(utils->data, "oplus,mdss-dsi-vspn-voltage", &panel->oplus_priv.vspn_voltage);
	if (ret) {
		DSI_INFO("oplus,mdss-dsi-vspn-voltage is not set, rc=%d\n", ret);
		panel->oplus_priv.vspn_voltage = 0;
	} else {
		DSI_INFO("oplus,mdss-dsi-vspn-voltage: %d", panel->oplus_priv.vspn_voltage);
	}

	dsi_panel_parse_oplus_apollo_config(panel);
...
int dsi_panel_parse_oplus_config(struct dsi_panel *panel)
{
	...
	panel->oplus_priv.is_samsung_panel = utils->read_bool(utils->data,
			"oplus,is_samsung_panel");
	DSI_INFO("is_samsung_panel: %s", panel->oplus_priv.is_samsung_panel ? "true" : "false");
...
********************************************/
	...
	panel->oplus_priv.display_off_set_pixel_off = utils->read_bool(utils->data,
		"oplus,display-off-set-pixel-off");
	DSI_INFO("oplus,display-off-set-pixel-off: %s",
		panel->oplus_priv.display_off_set_pixel_off ? "true" : "false");

	panel->oplus_priv.panel_physical_type = utils->get_property(utils->data,
		"oplus,mdss-dsi-physical-type", NULL);
	if (!panel->oplus_priv.panel_physical_type) {
		pr_info("Failed to parse panel physical type, set to default: oled\n");
		panel->oplus_priv.panel_physical_type = "oled";
	} else {
		if (strcmp(panel->oplus_priv.panel_physical_type, "lcd")
			&& strcmp(panel->oplus_priv.panel_physical_type, "oled")) {
			pr_info("Invaild panel physical type argument: %s, set to default: oled\n",
				panel->oplus_priv.panel_physical_type);
			panel->oplus_priv.panel_physical_type = "oled";
		} else {
			DSI_INFO("oplus,mdss-dsi-physical-type: %s", panel->oplus_priv.panel_physical_type);
		}
	}
	return 0;
}
...
bool sde_cp_crtc_update_pcc(struct drm_crtc *crtc)
{
	...
	bool pcc_skip_mode;
	int i = 0;
	num_mixers = (num_mixers > CRTC_DUAL_MIXERS) ? CRTC_DUAL_MIXERS : num_mixers;
...
int oplus_display_panel_set_dimlayer_hbm(void *data)
{
	...
	if (!strcmp(display->panel->oplus_priv.panel_physical_type, "lcd"))
		return 0;
