...
static int smb5_probe(struct platform_device *pdev)
{
	...
	rc = of_property_match_string(chg->dev->of_node, "io-channel-names", "smb1390_therm ");
	if (rc >= 0) {
		chg->iio.battcon_btb_chan = iio_channel_get(chg->dev, "smb1390_therm ");
		if (IS_ERR(chg->iio.battcon_btb_chan)) {
			rc = PTR_ERR(chg->iio.battcon_btb_chan);
			if (rc != -EPROBE_DEFER)
				dev_err(chg->dev, "battcon_btb_chan  get error, %ld\n", rc);
				chg->iio.battcon_btb_chan = NULL;
				return rc;
		}
